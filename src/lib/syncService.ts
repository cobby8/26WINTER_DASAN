            ...
            try {
                ...
                if (!uniqueClasses.has(key)) {
                    ...
                    if (classObj.originalText && classObj.originalText !== time) {
                        // Only append original text if it's significantly different? 
                        // Or just rely on the standard name which is cleaner.
                        // className += ` (${classObj.originalText})`; 
                        // User screenshot shows long names. Let's keep originalText logic but maybe cleaner?
